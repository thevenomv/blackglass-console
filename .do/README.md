...
App Platform specs in this repo use **`npm ci && npm run build`** for `build_command` and **`npm run start`** for `run_command` (see `app-current.yaml`, `app-git.*.yaml`, and the `web` service in `app-create.phase1.json`). That matches `package.json`: `build` is **`next build`**.
