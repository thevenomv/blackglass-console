...
| `app-create.phase1.json` | **Not in repo** — only needed if you run `scripts/do_bootstrap_blackglass.py` (create locally next to that script’s expectations, or use `doctl apps create --spec .do/app-git.production.yaml` instead). |
...
App Platform specs in this repo use **`npm ci && npm run build`** for `build_command` and **`npm run start`** for `run_command` (see `app-current.yaml` and `app-git.*.yaml`). That matches `package.json`: `build` is **`next build`**.
