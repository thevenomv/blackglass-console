...
import { RunScanButton } from "../dashboard/_components/RunScanButton";
import { BaselinesToolbar } from "./_components/BaselinesToolbar";
import { CaptureBaselineButton } from "./_components/CaptureBaselineButton";
...
function driftEventsToCategories(events: DriftEvent[]): BaselineDiffCategory[] {
  ...
  for (const event of events) {
    ...
    try {
      evidence = JSON.parse(event.evidenceSummary ?? "{}") as Record<string, unknown>;
    } catch {
      /* ignore */
    }
...
async function BaselineComparisonContent({ hostId }: { hostId: string }) {
  ...
  if (grouped.length === 0) {
    return (
      <EmptyState
        title="No drift versus baseline"
        description="Run a scan after capturing a baseline to populate this comparison."
        action={
...
export default async function BaselinesPage({
  ...
  const liveHostIds = live ? await listBaselineHostIds() : [];
  const hostId = host ?? liveHostIds[0] ?? "";
      ...
      <AppShell>
        <div className="flex flex-col gap-6 px-6 pb-12 pt-6">
          ...
          <EmptyState
            ...
            description={
              configuredCollectorHostIds().length
                ? "Capture a baseline while hosts are in a known-good state."
                : "Verify collector host env vars, then capture a baseline."
            }
            action={<CaptureBaselineButton variant="primary" />}
          />
        </div>
      </AppShell>
    );
        ...
        <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
          <aside className="h-fit rounded-card border border-border-default bg-bg-panel p-4 lg:sticky lg:top-24">
            ...
            <ul className="mt-3 space-y-2">
              {snapshots.length === 0 ? (
                <li className="text-sm text-fg-muted">No baseline stored for this host yet.</li>
              ) : (
                snapshots.map((s) => (
                  <li
                    key={s.id}
