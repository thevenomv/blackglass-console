...
> When the multi-user / Clerk-backed auth migration in `docs/saas-clerk-rbac.md` is finished, the console will use real per-user accounts with proper Clerk-managed forgot-password flows. Until then, the rotation paths below are the canonical recovery story.
