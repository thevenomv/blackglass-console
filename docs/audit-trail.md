...
API: **`GET /api/v1/audit/events`** returns the recent in-memory/list slice (`readAudit`). **Spaces JSONL is not streamed back automatically** — use object storage tooling for long-term retrieval.
