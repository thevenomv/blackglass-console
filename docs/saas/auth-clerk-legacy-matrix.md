...
Checkout (authenticated org) sends **`saas_tenant_id`** in session + subscription metadata. Webhooks call **`src/lib/saas/stripe-sync.ts`** to update `plan_code`, limits, Stripe IDs, and period fields. Schema lives in `src/db/schema/saas.ts` and is applied via `npm run db:migrate`.
