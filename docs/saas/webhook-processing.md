...
For stricter guarantees, move to **outbox pattern**: insert webhook row + domain events in one DB transaction; a worker applies domain events idempotently. This sketch matches the current “claim then process” model documented for operators in [docs/operations/operator-guide.md](operator-guide.md).
