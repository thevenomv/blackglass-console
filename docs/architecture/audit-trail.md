...
- **Retention:** per-plan window; legal/billing rows are kept for the
  account lifetime. See `docs/security/data-retention-saas.md`.
...
| **`AUDIT_DATABASE_URL` (PostgreSQL)** | Optional server-side append — run `docs/sql/audit-events.sql` once; runtime uses `src/lib/server/store/legacy/audit-append-pg.ts` (lazy `pg` pool). |
...
**Minimal schema** (`docs/sql/audit-events.sql`): `id` (uuid
