...
2. Run `npm run doppler:verify` after loading env, e.g. `node --env-file=.env.local scripts/secrets/doppler-verify.mjs` or set variables in your shell and `npm run doppler:verify`.
