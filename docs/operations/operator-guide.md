...
**End-to-end spine:** baseline capture → scan job → drift findings → investigation (UI / audit) → evidence/export surfaces. See **`docs/architecture/architecture-flow.md`** for how this maps to routes and `src/lib/server/` (including **`services/`** orchestration helpers).

**Staging / SaaS:** Before external pilots, run **`docs/operations/staging-deployment-checklist.md`** and **`npm run verify:staging`** with **`STAGING_URL`**. Longer-term product phases: **`docs/saas/saas-customer-roadmap.md`**. Revenue/checkout verification: **`docs/operations/wiring-checklist.md`**.
...
`src/lib/auth/legacy-permissions.ts`.
...
SaaS path is active. See `docs/saas/saas-clerk-rbac.md` and
`docs/saas/auth-clerk-legacy-matrix.md` for the full matrix.
...
4. **Webhook idempotency** (`saas_webhook_idempotency`) is safe to restore with the rest of the tenant snapshot — replayed Stripe/Clerk events will no-op on unique `(source, event_key)`. Schedule `npm run prune:webhooks` weekly (or `scripts/webhooks/prune-webhook-idempotency.mjs --days=30`) to cap table growth.
5. **Row-level security:** the canonical RLS policy set is enforced by `drizzle/0016_consolidate_rls_gucs.sql`, which standardises every shipped tenant policy on the `(app.tenant_id, app.bypass_rls)` GUC pair. The app sets these per transaction (`src/db/index.ts`). Ops scripts that delete across tenants (e.g. audit prune) must use `withBypassRls()` or a privileged role. See `docs/security/security-compliance.md` § 3.
...
| `rate_limited` from `/api/v1/scans`                                  | Too many scan requests — limit is 24 / 60 s per IP (see `docs/security/http-rate-limit-budgets.md`)                                                                                                       | Wait until the window resets, or back off                                               |
