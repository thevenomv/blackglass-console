...
- DigitalOcean (existing App Platform app): with **`DIGITALOCEAN_ACCESS_TOKEN`** set locally, run **`python scripts/do/do_apply_stage0.py`** to set **`AUTH_REQUIRED=true`** and ensure **`AUTH_SESSION_SECRET`** (generates one if missing). See script docstring for **`BLACKGLASS_APP_ID`** / **`AUTH_SESSION_SECRET`** overrides.
...
- [ ] **Runbook:** `docs/operations/operator-guide.md` and `docs/runbooks/operations.md` linked in the on-call channel.
...
- SOC 2 attestation, formal customer SLA, pen test sign-off — see
  `docs/saas/saas-customer-roadmap.md`.
