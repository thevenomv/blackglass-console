...
- Deploy with **`AUTH_REQUIRED=true`**, encrypted secrets (Doppler sync or env), **`NEXT_PUBLIC_USE_MOCK=false`**.
- Mount volumes for **`BASELINE_STORE_PATH`** / **`DRIFT_HISTORY_PATH`** if you care about restart survival.
- Run **`npm run test:unit`**, CI on **`main`**, and **`STAGING_URL=… node scripts/verify-staging.mjs`** after each deploy.
