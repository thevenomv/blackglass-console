...
| RLS bypass callsite tagging              | Every `withBypassRls(` call is preceded by `// RLS-BYPASS: <reason>`; PR CI enforces 1:1 counts | `npm run check:rls-bypass` · `scripts/build/check-rls-bypass-tags.mjs` · `src/db/index.ts` |
...
| Control                                | Implementation                                                                  | Verify here                                                                       |
| -------------------------------------- | ------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| Append-only audit table                | `saas_audit_events` — every privileged action logged with actor + target         | `src/db/schema/saas.ts` (`saasAuditEvents`), `src/lib/server/audit-log.ts`         |
...
| Rate limiting                          | Redis-backed sliding window, per-IP + per-tenant budgets                                | `src/lib/server/rate-limit.ts`, `docs/security/http-rate-limit-budgets.md`                 |
...
| Lockfile hygiene                       | `package-lock.json` committed; `npm ci` used in production builds                | `scripts/build/build-worker.mjs`, DO App Spec                                      |
| Pre-commit gates                       | `npm run lint && npm run typecheck && npx vitest run` required before push       | `docs/operations/release-checklist.md`                                                       |
...
| Incident notification policy           | Customer-facing incident notification SLA + template                              | `docs/security/incident-notification.md`                                                   |
...
| Scan execution off web thread          | BullMQ `blackglass-janitor` consumer in `src/worker/ops/`                          | `src/worker/ops/index.ts`, `janitor-queue.ts`                                    |
...
See [`docs/architecture/vendor-inventory.md`](./vendor-inventory.md) for the full
...
Obsidian sub-processors solely for that feature (see `docs/architecture/vendor-inventory.md`).
...
- **DAST is scheduled, not gating.** A ZAP baseline workflow runs against
  ...
  by default. Tune via `docs/security/zap-baseline-rules.md`.
