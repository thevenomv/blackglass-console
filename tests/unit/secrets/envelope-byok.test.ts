...
describe("envelope BYOK routing", () => {
  it("round-trips through the global KEK when BYOK is off (no tenantId on blob)", async () => {
    const { encryptKey, decryptKey } = await import("@/lib/server/secrets/envelope");
  ...
  it("ignores BYOK_ENABLED when no tenant row exists (lookup returns null)", async () => {
    ...
    vi.doMock("@/lib/server/secrets/tenant-kms", async () => {
      const real = await vi.importActual<typeof import("@/lib/server/secrets/tenant-kms")>(
        "@/lib/server/secrets/tenant-kms",
      );
      return {
        ...real,
        byokEnabled: () => true,
  ...
  it("decrypt rejects a tenant-tagged blob when BYOK is later disabled", async () => {
    ...
    vi.doMock("@/lib/server/secrets/tenant-kms", () => ({
      byokEnabled: () => false,
      loadTenantKmsConfig: vi.fn().mockResolvedValue(null),
    }));
    const { decryptKey } = await import("@/lib/server/secrets/envelope");
    await expect(
      decryptKey("tenant-a", {
        ciphertext: "ignored",
        wrappedDek: "ignored",
